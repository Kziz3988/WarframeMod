...
{
    [HarmonyPrefix]
    [HarmonyPatch(nameof(CardModel.IsValidTarget))]
    static bool IsValidTargetPatch(CardModel __instance, Creature? target, ref bool __result)
    {
    ...
    [HarmonyPrefix]
    [HarmonyPatch(nameof(CardModel.OverlayPath), MethodType.Getter)]
    static bool OverlayPathPatch(CardModel __instance, ref string __result)
    {
        if (__instance is WarframeModCard)
        {
            WarframeModCard card = __instance as WarframeModCard;
            __result = card.CustomOverlayPath;
            return false;
        }
        return true;
    }
